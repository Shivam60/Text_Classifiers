...
test=os.getcwd()+r"/input/test/"
def process_folder(arg):
    sub=os.listdir(test+arg)
    for subfiles in sub:
        with open(test+arg+r'/'+subfiles,'r') as file1:
            sent=file1.read()
            file1.close()
...
if __name__ == '__main__':
    ...
    results = pool.map(process_folder,itter)
    print results
